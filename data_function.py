...
class MedData_train(torch.utils.data.Dataset):
    def __init__(self, images_dir, labels_dir,orien_dir):
        if hp.mode == '3d':
        ...
        labels_dirs = Path(labels_dir)
        self.label_paths = sorted(labels_dirs.glob(hp.fold_arch))
        ori_dir = Path(orien_dir)
        self.ori_paths = sorted(ori_dir.glob(hp.fold_arch))
        for (image_path, labels_path,orien_path) in zip(self.image_paths, self.label_paths,self.ori_paths):
            subject = tio.Subject(
                img=tio.ScalarImage(image_path),
                vessel=tio.LabelMap(labels_path),
                oriention=tio.LabelMap(orien_path),
            )
            self.subjects.append(subject)
        # for (image_path, labels_path,) in zip(self.image_paths, self.label_paths):
        #     subject = tio.Subject(
        #         img=tio.ScalarImage(image_path),
        #         vessel=tio.LabelMap(labels_path),
        #     )
